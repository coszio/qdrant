...
use segment::vector_storage::query::{
    ContextQuery, DiscoveryQuery, FeedbackQuery, LinearFeedbackFormula, RecoQuery,
};
use serde::Serialize;
...
pub enum QueryEnum {
    ...
    FeedbackLinear(NamedQuery<FeedbackQuery<VectorInternal, LinearFeedbackFormula>>),
}
...
impl From<QueryEnum> for grpc::QueryEnum {
    fn from(value: QueryEnum) -> Self {
        match value {
            ...
            QueryEnum::FeedbackLinear(named) => grpc::QueryEnum {
                query: Some(grpc::query_enum::Query::Feedback(grpc::FeedbackQuery {
                    target: Some(named.query.target.into()),
                    feedback_pairs: named
                        .query
                        .feedback_pairs
                        .into_iter()
                        .map(|pair| grpc::FeedbackPair {
                            positive: Some(pair.positive.into()),
                            negative: Some(pair.negative.into()),
                    ...
                    formula: Some(grpc::FeedbackFormula {
                        variant: Some(grpc::feedback_formula::Variant::Linear(
                            named.query.formula.into(),
                        )),
                    }),
                })),
            },
