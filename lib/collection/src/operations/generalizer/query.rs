...
impl<T: Generalizer, TFormula: Generalizer> Generalizer for FeedbackQuery<T, TFormula> {
    fn remove_details(&self) -> Self {
        let FeedbackQuery {
            target,
            feedback_pairs,
        ...
        Self {
            ...
            formula: formula.remove_details(),
        }
    }
}
...
impl Generalizer for LinearFeedbackFormula {
    fn remove_details(&self) -> Self {
        let LinearFeedbackFormula { a: _, b: _, c: _ } = self;
        Self {
            a: 0.0.into(),
            b: 0.0.into(),
            c: 0.0.into(),
